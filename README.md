...
v75 menambah penyegerakan laporan sehala Supabase → Google Sheets. Supabase kekal satu-satunya data induk; tab `MURID`, `MARKAH_UJIAN` dan `PBD` dalam Sheet dikemas kini tanpa mencipta salinan data yang perlu diselenggara secara manual. Lihat `GOOGLE_SHEETS_SYNC.md` untuk pengaktifan sekali sahaja.
...
31. Penyegerakan Google Sheets v75: Supabase kekal sebagai data induk; `MURID`, `MARKAH_UJIAN` dan `PBD` disalin sehala ke Google Sheet melalui Edge Function yang mengesahkan akses Google kepada helaian sasaran. Skrip terikat Google Sheet menyediakan sync manual dan pencetus automatik setiap 5 minit tanpa menyimpan service-role key di browser, GitHub atau Sheet.
32. Pengiraan PBD v76: `PURATA TEMA` dikira daripada semua SP yang telah diisi bagi tema tersebut, manakala `TP AKHIR` dikira daripada purata tema yang telah mempunyai nilai. Paparan purata tidak lagi menunjukkan pecahan kelengkapan seperti `2/3` atau perpuluhan; nilai dipaparkan sebagai TP nombor bulat, manakala pecahan masih tersedia sebagai tooltip untuk rujukan.
...
- Folder `supabase/functions/google-sheets-sync` ialah kod server-side dan bukan fail rahsia. Deploy fungsi ini ke projek Supabase `iSEJARAH`; service-role key dibekalkan secara automatik oleh persekitaran Edge Function dan tidak boleh disalin ke GitHub.
- Untuk penyegerakan berkala, ikuti `GOOGLE_SHEETS_SYNC.md`. Kod dalam `google-sheets/` dipasang sekali sahaja sebagai Apps Script terikat pada Google Sheet sasaran.
...
## Google Sheets sebagai paparan sehala

- Supabase ialah satu-satunya sumber data. Guru tidak perlu mengisi data kali kedua dalam Google Sheet.
- Google Sheet tidak menghantar perubahan kembali ke Supabase, bagi mengelakkan konflik dan rekod berganda.
- Endpoint `google-sheets-sync` hanya menerima token Google sementara yang mempunyai akses kepada spreadsheet iSEJARAH yang tepat.
- Apps Script mengemas kini data sumber, manakala `PBD T4`, `PBD T5`, `PBD T6` dan `MARKAH KELAS` kekal sebagai paparan formula/laporan.
