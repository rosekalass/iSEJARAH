    ...
    function renderMarksModule() {
        ...
        if(examDateInput){
            ...
            examDateInput.classList.add('hidden');
        ...
        if(examDateLabelText)examDateLabelText.textContent='Tarikh Ujian · Paparan Sahaja';
        dateCard?.classList.add('is-readonly');
        if(examDateNote){
            ...
            examDateNote.textContent=actualDate
                ? 'Tarikh ini diselaraskan daripada Tetapan Sekolah dan digunakan untuk Markah serta PBD.'
                : 'Belum ditetapkan. Admin perlu menetapkannya di Tetapan Sekolah.';
    ...
    async function saveCurrentAssessmentDate(value){
        showAlert('Paparan Sahaja','Tarikh ujian hanya boleh diubah oleh Admin melalui Tetapan Sekolah › Kawalan Pengisian Markah & PBD.','info');
        renderMarksModule();
        return;

        /* Legacy path retained below for backward-compatible parsing; it is unreachable by design. */
    ...
    function formatPbdAverage(avg) {
        if(avg===null || avg===undefined || !Number.isFinite(Number(avg))) return '—';
        return String(Math.max(1,Math.min(6,Math.round(Number(avg)))));
    }
    ...
    function calculateStudentThemeAverage(studentId,group,year,period,academicYear=null) {
        ...
        // A theme average is meaningful as soon as at least one SK has been
        // entered. `complete` remains available for completion filters, but
        // the value shown to teachers reflects all entered SP values.
        const rawAvg=recorded.length?recorded.reduce((a,b)=>a+b,0)/recorded.length:null;
        const avg=rawAvg===null?null:Math.max(1,Math.min(6,Math.round(rawAvg)));
        return {complete,recorded:recorded.length,total:dskps.length,hasValues:recorded.length>0,rawAvg,avg};
    }
    ...
    function calculateStudentFinalAverage(studentId,template,year,period,academicYear=null) {
        ...
        const enteredThemes=themes.filter(x=>x.rawAvg!==null);
        const rawAvg=enteredThemes.length?enteredThemes.reduce((sum,x)=>sum+x.rawAvg,0)/enteredThemes.length:null;
        const avg=rawAvg===null?null:Math.max(1,Math.min(6,Math.round(rawAvg)));
        return {complete,hasValues:enteredThemes.length>0,rawAvg,avg,themes};
    }
        ...
        body.innerHTML=students.map((student,index)=>{
            ...
            template.groups.forEach((group,gIndex)=>{
                ...
                cells+=`<td data-theme-key="${group.key}" class="pbd-average-cell" onclick="highlightPbdStudentRow(this)">
                    ${themeCalc.rawAvg!==null
                        ? `<span class="pbd-average-chip ${pbdAvgClass(themeCalc.rawAvg)}" title="Purata daripada ${themeCalc.recorded} SP yang telah diisi">TP${formatPbdAverage(themeCalc.rawAvg)}</span>`
                        : `<span class="pbd-tp-pill pbd-tp-pill-empty">—</span>`}
                </td>`;
            });

            const finalHtml=finalCalc.rawAvg!==null
                ? `<span class="pbd-average-chip ${pbdAvgClass(finalCalc.rawAvg)}" title="Purata daripada ${finalCalc.themes.filter(x=>x.rawAvg!==null).length} Tema yang telah diisi">TP${formatPbdAverage(finalCalc.rawAvg)}</span>`
                : `<span class="pbd-tp-pill pbd-tp-pill-empty">—</span>`;

            return `<tr data-pbd-student="${student.id}" data-name="${escapeHtml(student.name.toLowerCase())}" data-complete="${finalCalc.complete?'1':'0'}" data-final-tp="${finalCalc.rawAvg!==null?finalCalc.avg:''}" onclick="highlightPbdStudentRow(this)">
    ...
    function renderAnalyticsCharts(scoredRows) {
        ...
        if (gradeCanvas) analyticsGradeChartInstance=new Chart(gradeCanvas,{type:'bar',data:{labels:gradeLabels,datasets:[{label:'Peratus Calon (%)',data:gradeLabels.map(g=>gradePercentages[g]),backgroundColor:['#0D9488','#06B6D4','#2563EB','#7C3AED','#DB2777','#E11D48'],borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:(ctx)=>`${ctx.raw}% (${gradeCounts[ctx.label]||0} murid)`}}},scales:{y:{beginAtZero:true,max:100,ticks:{callback:v=>v+'%'}}},onClick:(evt,elements)=>{if(elements.length){const g=gradeLabels[elements[0].index];setAnalyticsGradeFilter(g);}}}});
        if (bandCanvas) analyticsBandChartInstance=new Chart(bandCanvas,{type:'doughnut',data:{labels:['0–39','40–49','50–64','65–79','80–100'],datasets:[{data:bandCounts,backgroundColor:['#E11D48','#DB2777','#7C3AED','#2563EB','#0D9488'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{boxWidth:8,usePointStyle:true,font:{size:10}}}}}});
        ...
        if (bId !== 'NONE') {
            ...
            if(compareCanvas) analyticsCompareChartInstance=new Chart(compareCanvas,{type:'bar',data:{labels:['Purata'],datasets:[{label:a?.name||'A',data:[Number(avgA.toFixed(1))],backgroundColor:'#1677FF',borderRadius:6},{label:b?.name||'B',data:[Number(avgB.toFixed(1))],backgroundColor:'#13CFC9',borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,max:100}},plugins:{legend:{position:'bottom',labels:{font:{size:10}}}}}});
    ...
    function renderHeadcountStudentChart(row){
        ...
        hcCharts.student=new Chart(canvas,{type:'line',data:{labels:['TOV','OT1','AR1','OT2','AR2','ETR'],datasets:[
            {label:'Sasaran',data:[row.toy.value,row.oti1,row.oti1,row.oti2,row.oti2,row.etr],borderColor:'#23D5FF',borderDash:[4,3],pointRadius:3,tension:.25,spanGaps:true},
            {label:'Pencapaian',data:[row.toy.status==='VALUE'?row.toy.value:null,null,row.ar1.status==='VALUE'?row.ar1.value:null,null,row.ar2.status==='VALUE'?row.ar2.value:null,null],borderColor:'#D6AA52',pointRadius:4,tension:.25,spanGaps:true}
    ...
    function renderStudentProfileCharts(d) {
        ...
        studentProfileMarksChartInstance=new Chart(markCanvas,{type:'line',data:{labels,datasets:[{label:'Peratus',data:vals,borderColor:'#1677FF',backgroundColor:'rgba(35,213,255,.12)',fill:true,tension:.3,pointRadius:4,pointBackgroundColor:'#23D5FF'}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,max:100}},plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>formatWholePercent(c.raw)}}}}});
        const dist=[1,2,3,4,5,6].map(tp=>d.pbdRecorded.filter(x=>Number(x.record.tp)===tp).length);
        studentProfilePbdChartInstance=new Chart(document.getElementById('profile-pbd-chart'),{type:'doughnut',data:{labels:['TP1','TP2','TP3','TP4','TP5','TP6'],datasets:[{data:dist,backgroundColor:['#E11D48','#F97316','#F59E0B','#2563EB','#7C3AED','#0D9488'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:10}}}}}});
    ...
    function renderEntryControlSettings() {
        ...
        renderCentralizedAssessmentDateSettings();
    }

    function previewCentralizedAssessmentDate(type) {
        const input=document.getElementById(`entry-${type}-date`);
        const note=document.getElementById(`entry-${type}-date-note`);
        if(!input||!note)return;
        const value=normalizeAssessmentExamDate(input.value);
        note.textContent=value ? formatAssessmentExamDate(value) : (input.dataset.mixed==='true'?'Pelbagai tarikh — pilih untuk selaraskan':'Belum ditetapkan');
        note.classList.toggle('is-set',Boolean(value));
    }

    function renderCentralizedAssessmentDateSettings() {
        const academicYear=getActiveAcademicYear();
        ['DIAGNOSTIK','UPSA','UASA'].forEach(type=>{
            const input=document.getElementById(`entry-${type}-date`);
            if(!input)return;
            const dates=[...new Set(appState.assessments
                .filter(a=>a.type===type&&String(a.academicYear||academicYear)===academicYear)
                .map(a=>assessmentExamDateValue(a)).filter(Boolean))];
            input.value=dates.length===1?dates[0]:'';
            input.dataset.mixed=dates.length>1?'true':'false';
            previewCentralizedAssessmentDate(type);
        });
    }

    function applyCentralizedAssessmentDates(academicYear) {
        ensureAutoAssessmentTemplates(false,academicYear);
        let changed=false;
        const pbdDates={};
        ['DIAGNOSTIK','UPSA','UASA'].forEach(type=>{
            const input=document.getElementById(`entry-${type}-date`);
            if(!input)return;
            const requested=normalizeAssessmentExamDate(input.value);
            if(!requested&&input.dataset.mixed==='true')return;
            if(type==='UPSA')pbdDates.PERTENGAHAN=requested;
            if(type==='UASA')pbdDates.AKHIR=requested;
            appState.assessments
                .filter(a=>a.type===type&&String(a.academicYear||academicYear)===String(academicYear))
                .forEach(assessment=>{
                    if(assessmentExamDateValue(assessment)===requested)return;
                    assessment.date=requested;
                    assessment.updatedAt=new Date().toISOString();
                    assessment.updatedBy=currentUserId;
                    changed=true;
                    if(phase10Mode==='SUPABASE')phase10Upsert('assessments',assessment.id,assessment,{subject:'SEJARAH'}).catch(err=>console.error('Assessment date sync:',err));
                });
        });
        let pbdChanged=false;
        appState.pbdRecords
            .filter(record=>String(record.academicYear||academicYear)===String(academicYear)&&Object.prototype.hasOwnProperty.call(pbdDates,record.assessmentPeriod))
            .forEach(record=>{
                const requested=pbdDates[record.assessmentPeriod];
                if(normalizeAssessmentExamDate(record.assessmentDate)===requested)return;
                record.assessmentDate=requested;
                record.updatedAt=new Date().toISOString();
                record.updatedBy=currentUserId;
                pbdChanged=true;
                if(phase10Mode==='SUPABASE')phase10Upsert('pbdRecords',record.id,record).catch(err=>console.error('PBD date sync:',err));
            });
        if(changed)persistMarksState();
        if(pbdChanged)persistPhase4State();
        return changed||pbdChanged;
    }
    ...
    function savePhase9SchoolProfile(){
        ...
        persistPhase9State();
        const assessmentDatesChanged=applyCentralizedAssessmentDates(active);
        ...
        if(typeof renderDashboardGpmpSummary==='function')renderDashboardGpmpSummary();
        if(typeof updateDashboardKPIs==='function')updateDashboardKPIs();
        showAlert('Tetapan Disimpan',`Sesi akademik aktif kini ${phase9AyLabel(active)}. Kaedah Headcount ${headcountMethod==='METHOD2'?'2':'1'} digunakan${assessmentDatesChanged?' dan tarikh ujian telah diselaraskan untuk semua kelas':''}.`,'success');
    }
    ...
    function renderClassComparisonCharts(metrics) {
        ...
        classCompareMarksChartInstance = new Chart(marksCanvas, {
            type:'bar',
            data:{ labels, datasets:[
                {label:'GPMP (%)',data:metrics.map(m=>m.average===null?null:Number(m.average.toFixed(1))),backgroundColor:'#1677FF',borderRadius:6},
                {label:'Menguasai (%)',data:metrics.map(m=>m.masteryRate===null?null:Number(m.masteryRate.toFixed(1))),backgroundColor:'#13CFC9',borderRadius:6}
            ]},
        ...
        classComparePbdChartInstance = new Chart(pbdCanvas, {
            type:'bar',
            data:{ labels, datasets:[
                {label:'PBD Lengkap (%)',data:metrics.map(m=>m.pbdCompletion===null?null:Number(m.pbdCompletion.toFixed(1))),backgroundColor:'#23D5FF',borderRadius:6,yAxisID:'y'},
                {label:'Purata TP',data:metrics.map(m=>m.avgTp===null?null:Number(m.avgTp.toFixed(2))),backgroundColor:'#D6AA52',borderRadius:6,yAxisID:'y1'}
            ]},
            options:{responsive:true,maintainAspectRatio:false,scales:{
                x:{ticks:{autoSkip:false,maxRotation:0,minRotation:0,font:{size:10}}},
    ...
    function refreshResponsiveCharts(){
        ...
        responsiveResizeTimer=setTimeout(()=>{
            const charts=[
                ...
                analyticsGradeChartInstance,analyticsBandChartInstance,analyticsCompareChartInstance,
                classCompareMarksChartInstance,classComparePbdChartInstance,
                pbdAnalyticsTpChartInstance,pbdAnalyticsSpChartInstance,
                studentProfileMarksChartInstance,studentProfilePbdChartInstance,
                ...Object.values(hcCharts)
            ];
