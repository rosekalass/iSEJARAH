    ...
    function renderAnalyticsCharts(scoredRows) {
        ...
        if (gradeCanvas) analyticsGradeChartInstance=new Chart(gradeCanvas,{type:'bar',data:{labels:gradeLabels,datasets:[{label:'Peratus Calon (%)',data:gradeLabels.map(g=>gradePercentages[g]),backgroundColor:['#13CFC9','#23D5FF','#1677FF','#D6AA52','#9A7735'],borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:(ctx)=>`${ctx.raw}% (${gradeCounts[ctx.label]||0} murid)`}}},scales:{y:{beginAtZero:true,max:100,ticks:{callback:v=>v+'%'}}},onClick:(evt,elements)=>{if(elements.length){const g=gradeLabels[elements[0].index];setAnalyticsGradeFilter(g);}}}});
        if (bandCanvas) analyticsBandChartInstance=new Chart(bandCanvas,{type:'doughnut',data:{labels:['0–39','40–49','50–64','65–79','80–100'],datasets:[{data:bandCounts,backgroundColor:['#8A6A30','#D6AA52','#1677FF','#23D5FF','#13CFC9'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{boxWidth:8,usePointStyle:true,font:{size:10}}}}}});
    ...
    function renderPbdAnalyticsCharts(m) {
        ...
        pbdAnalyticsTpChartInstance=new Chart(tpCanvas,{type:'doughnut',data:{labels:['TP1','TP2','TP3','TP4','TP5','TP6'],datasets:[{data:m.counts,backgroundColor:['#8A6A30','#D6AA52','#4E97FF','#1677FF','#23D5FF','#13CFC9'],borderColor:'#F7F9FC',borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{usePointStyle:true,boxWidth:8,font:{size:11}}}}}});
    ...
    function renderStudentProfileCharts(d) {
        ...
        studentProfilePbdChartInstance=new Chart(document.getElementById('profile-pbd-chart'),{type:'doughnut',data:{labels:['TP1','TP2','TP3','TP4','TP5','TP6'],datasets:[{data:dist,backgroundColor:['#8A6A30','#D6AA52','#4E97FF','#1677FF','#23D5FF','#13CFC9'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:10}}}}}});
